...
async function starts() {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			const ownerNumber = ["5511934660977@s.whatsapp.net"] // Nomer Owner
            ...
            break
              ...
              case 'antilinkgroup':
					...
					if (Number(args[0]) === 1) {
						...
						nzwa.sendMessage(from,`❗Atenção a todos os membros do grupo❗ o anti-link está ativo, quando você enviar algum link automaticamente será expulso do grupo`, text)
					} else if (Number(args[0]) === 0) {
						if (!isantilink) return reply('O anti-link foi desativado. ❗Grupo vuneravel❗')
						var ini = anti.indexOf(from)
						antilink.splice(ini, 1)
						fs.writeFileSync('./database/json/antilink.json', JSON.stringify(antilink))
						reply('Desativado com sucesso o anti-link neste grupo ✔️')
					} else {
